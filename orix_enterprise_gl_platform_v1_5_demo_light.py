...
class MainWindow(QMainWindow):
    ...
    def _page_report(self) -> QWidget:
        ...
        self.lbl_sccl_trade_restricted = QLabel("Trade-level view restricted by role/entitlement. Switch to Maker/Checker to view atomic trades.")
        self.lbl_sccl_trade_restricted.setStyleSheet("color:#8A4B00;")
        self.lbl_sccl_trade_restricted.setVisible(False)
        r2.addWidget(self.lbl_sccl_trade_restricted)
    ...
    def _page_queue(self) -> QWidget:
        """Enterprise work queues with routing and bulk triage."""
        w = QWidget()
        layout = QVBoxLayout(w)

        hdr = QLabel("Work Queues & Routing")
        hdr.setFont(QFont("Arial", 14, QFont.Weight.Bold))
        layout.addWidget(hdr)

        hint = QLabel("Unified action queue across feeds, recon breaks, variances, SCCL. Routed by domain/team with SLA.")
        hint.setStyleSheet("color:#5A6772;")
        layout.addWidget(hint)

        bar = QHBoxLayout()
        bar.addWidget(QLabel("Queue:"))
        ...
        layout.addWidget(self.tbl_queue, 1)
        return w
    ...
    def refresh_queue(self):
        ...
        sel = self.cmb_queue.currentText() if hasattr(self, "cmb_queue") else "All"
        if sel and sel != "All" and not q.empty:
            q = q[q["domain"] == sel].copy()
